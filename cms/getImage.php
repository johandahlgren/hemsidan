...
if (file_exists($filename))
{
	...
	if ($width > 0 && $height > 0)
	{
		...
		$resized = "../images/cache/" . $tn_width . "x" . $tn_height . "-" . $image;
		...
		{ 
			case 'jpg': 
				$src = imagecreatefromjpeg($filename) or notfound();
				break;
			case 'JPG': 
				$src = imagecreatefromjpeg($filename) or notfound();
				break;
			case 'png': 
				$src = imagecreatefrompng($filename) or notfound();
				break;
			case 'PNG': 
				$src = imagecreatefrompng($filename) or notfound();
				break;
			case 'gif':
				$src = imagecreatefromgif($filename) or notfound();
				break;
			case 'GIF':
				$src = imagecreatefromgif($filename) or notfound();
				break;
		...
		if (function_exists("imageconvolution")) 
		{
			...
			$divisor 	= 24;
			$offset 	= 0;
			imageconvolution($dst, $matrix, $divisor, $offset); // Antialias
		}
